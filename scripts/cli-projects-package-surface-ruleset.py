...
def validate_workflow(path=WORKFLOW):
    ...
    require(set(document.get("jobs", {})) == {"package-surface"},
            "required workflow must expose one package-surface job")
    job = document["jobs"]["package-surface"]
    require(job.get("if") == "github.repository == 'Verjson/verjson-cli-projects'",
    ...
    require("secrets." not in serialized and "secrets: inherit" not in serialized,
            "required workflow may not receive consumer secrets")
...
def resolve_existing_org_rule(named, expected, staged):
...
def activate_repository_rule(contract):
...
def main(arguments=None):
    ...
    if args.mode == "activate-repository":
        activate_repository_rule(contract)
        print("activated and verified repository ruleset 21567958")
        return 0
    staged = dict(expected)
    staged["enforcement"] = "disabled"
    if named:
        ruleset_id, state = resolve_existing_org_rule(named, expected, staged)
        if state == "active":
            print(f"verified existing active organization ruleset {ruleset_id}")
            return 0
    else:
        appeared = discover_state(contract, args.workflow_sha)
        if appeared:
            ruleset_id, state = resolve_existing_org_rule(appeared, expected, staged)
            if state == "active":
                print(f"verified existing active organization ruleset {ruleset_id}")
                return 0
        else:
