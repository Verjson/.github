...
if [ "${#release_approved_internal_packages[@]}" -gt 0 ] && [ "$mode" != release-artifact ]; then
  echo "$(basename "$0"): --approved-internal-package is accepted only by release-artifact" >&2
  exit 2
fi
...
${build_runners_yaml%$'\n'}
    ...
    steps:
      ...
      - name: Save exact-attempt credentialless dependencies
        ...
        with:
          path: node_modules
          key: release-dependencies-\${{ github.run_id }}-\${{ github.run_attempt }}-\${{ strategy.job-index }}
