...
emit_contract_test() {
  ...
  adr_index_test_sha256="$(emit_adr_index_test 2>/dev/null | digest_of)" || true
...
case "$mode" in
  ...
  adr-index-generator)
    ...
    ;;
  adr-index-test)
    out="$(emit_adr_index_test)" \
      || { echo "$(basename "$0"): cannot resolve gen-adr-index.test.sh at $ref" >&2; exit 1; }
    printf '%s\n' "$out" | bash -n 2>/dev/null \
