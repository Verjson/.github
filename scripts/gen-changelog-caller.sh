...
generated_set_check() { # generated_set_check <relative-path> <required|optional> <pin-pattern> <generator-mode> [generator-flags] [remedy-note]
  local rel="$1" required="$2" pattern="$3" mode="$4" flags="${5:-}" note="${6:-}"
  local abs="$root/$rel" claims claim count remedy
...
generated_set_check .github/workflows/changelog.yml           required "$generated_set_header_pin" generated-artifacts '' " (or the workflow compatibility alias)"
generated_set_check .github/workflows/changelog-contract.yml  required "$generated_set_header_pin" pr-gate
...
# split generated-artifacts caller, a release caller, a release proposer or the
# ADR index legitimately does not carry the file. Once present, it is held to
# the same pin as everything else.
generated_set_check .github/workflows/generated-artifacts.yml optional "$generated_set_header_pin" generated-artifacts-with-adr-index
generated_set_check .github/workflows/renovate-changelog.yml  optional "$generated_set_header_pin" renovate-attribution
generated_set_check .github/workflows/release.yml             optional "$generated_set_header_pin" release-node '' " (or release-artifact for GitHub Release assets, or release-snapshot when the repository publishes nothing from the release workflow)"
generated_set_check .github/workflows/release-propose.yml     optional "$generated_set_header_pin" release-propose "--autonomy propose"
generated_set_check scripts/gen-adr-index.test.sh             optional "$generated_set_comment_pin" adr-index-test
...
validate_adr_generator() {
  ...
  [ -n "$ADR_INDEX_TEST_SHA256" ] \
    || fail "adr-index: true has no canonical generator test at $CONTRACT_REF"
  ...
  [ "$(contract_digest_of "$adr_generator_test")" = "$ADR_INDEX_TEST_SHA256" ] \
    || fail "$adr_generator_test is not the generator test pinned at $CONTRACT_REF. Replace any hand-written copy with: scripts/gen-changelog-caller.sh adr-index-test $CONTRACT_REF > scripts/gen-adr-index.test.sh"
}
