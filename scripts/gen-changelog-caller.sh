...
usage() {
  echo "usage: $(basename "$0") {workflow|generated-artifacts|generated-artifacts-with-adr-index|adr-index-generator|renderer|contract-test|release-node|release-propose} <40-hex-commit> [--scope <npm-scope>] [--node-version <version>] [--package-dir <relative-dir>]... [--autonomy {propose|dispatch}]" >&2
  exit 2
}
...
while IFS= read -r release_workflow; do
  ...
  grep -qF 'run-name: Release ${{ inputs.version }}' "$release_workflow" \
