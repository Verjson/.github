...
usage() {
  echo "usage: $(basename "$0") {workflow|generated-artifacts|generated-artifacts-with-adr-index|adr-index-generator|renderer|contract-test|release-node} <40-hex-commit>" >&2
  exit 2
}
...
while IFS= read -r release_workflow; do
  ...
  stamp_before() {
    ...
    # release-stripped.yml removes shell `${VERSION#v}` from the first `#`;
    # matching the command prefix here avoids mistaking that shell expansion
    # for a YAML comment boundary.
    stamp_line="$(printf '%s\n' "$job" | grep -n -m1 'npm version ' | cut -d: -f1)"
