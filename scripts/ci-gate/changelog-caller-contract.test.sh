...
cmp -s <(printf '%s\n' "$adr_index_test" | tail -n +12) <(printf '%s\n' "$expected_adr_index_test") \
...
if run_adopter "$unknown_release"; then
  ...
  unknown_release_finding="$(grep -F '.github/workflows/release.yml is not a generated release caller' "$tmproot/run.out")"
  unknown_release_guidance="${unknown_release_finding#* at $sha. }"
  expected_unknown_release_guidance="Inspect the existing release workflow and repository configuration to determine its mode and all custom generator options. Regenerate the complete caller set at $sha in a clean temporary checkout, review the full diff, then replace the committed set together. Supported release modes are release-node, release-artifact for GitHub Release assets, and release-snapshot when the release workflow publishes nothing."
  [ "$unknown_release_guidance" = "$expected_unknown_release_guidance" ] && [ -s "$unknown_release/.github/workflows/release.yml" ] \
  && pass "unrecognized release provenance gives exact prose-only guidance and leaves workflow intact" \
  || fail "unrecognized release provenance guidance differed: $unknown_release_guidance"
fi
