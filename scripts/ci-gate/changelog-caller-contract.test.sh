...
printf '%s\n' "$adr_index_test" | grep -qF 'repo_root="$(cd "$here/.." && pwd)"' \
  && ! printf '%s\n' "$adr_index_test" | grep -qF 'repo_root="$(cd "$here/../.." && pwd)"' \
...
printf '%s\n' "$adr_index_test" | grep -qE "^# CONTRACT_REF=$sha\$" \
...
grep -qF 'scripts/gen-changelog-caller.sh adr-index-test $CONTRACT_REF > scripts/gen-adr-index.test.sh' "$emitted" \
  && pass "the contract test names how to acquire the pinned ADR-index suite" \
  || fail "the contract test does not name the adr-index-test acquisition command"
