...
def contract(candidate_gate=gate, candidate_arm=arm, candidate_policy=policy):
    checks = {
        ...
        "persistent opt-in is authenticated and consumed before spend": 'authorize_ai_review_actor "$ai_review_actor"' in candidate_arm and 'echo "explicit_ai_review=$explicit_ai_review"' in candidate_arm and 'authorization_label=ai-review' in candidate_arm and 'Review label consumption failed' in candidate_arm and candidate_arm.index('Review label consumption failed') < candidate_arm.index('gh workflow run ai-review-merge.yml'),
        ...
        "operator recovery authorizes and consumes a current re-review label": '[ "$operator_recovery" = true ] && [ "$explicit_rereview" = false ] && [ -n "$rereview_label" ]' in candidate_arm and 'could not resolve the current re-review label actor during operator recovery' in candidate_arm and 'steps.arm.outputs.explicit_rereview || false' in candidate_arm and 'authorization_label=re-review' in candidate_arm and candidate_arm.index('Review label consumption failed') < candidate_arm.index('gh workflow run ai-review-merge.yml'),
...
mutations = {
    ...
    "persistent opt-in not consumed": (gate, arm.replace('authorization_label=ai-review', 'authorization_label='), policy),
    ...
    "operator recovery ignores current re-review label": (gate, arm.replace('[ "$operator_recovery" = true ] && [ "$explicit_rereview" = false ] && [ -n "$rereview_label" ]', 'false'), policy),
    "operator recovery dispatches before consuming label": (gate, arm.replace('authorization_label=re-review', 'authorization_label='), policy),
