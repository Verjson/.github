...
def contract(candidate_gate=gate, candidate_arm=arm, candidate_policy=policy):
    checks = {
        ...
        "explicit label delivery is authenticated and consumed before spend": 'authorize_ai_review_actor "$REQUEST_ACTOR"' in candidate_arm and 'echo "explicit_ai_review=$explicit_ai_review"' in candidate_arm and 'authorization_label=ai-review' in candidate_arm and 'Review label consumption failed' in candidate_arm and candidate_arm.index('Review label consumption failed') < candidate_arm.index('gh workflow run ai-review-merge.yml'),
...
mutations = {
    ...
    "explicit opt-in not consumed": (gate, arm.replace('authorization_label=ai-review', 'authorization_label='), policy),
