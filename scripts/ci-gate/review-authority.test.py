...
def contract(candidate_gate=gate, candidate_arm=arm, candidate_policy=policy):
    checks = {
        ...
        "operator recovery authorizes and consumes a current re-review label": '[ "$operator_recovery" = true ] && [ "$explicit_rereview" = false ] && [ -n "$rereview_label" ]' in candidate_arm and 'could not resolve the current re-review label actor during operator recovery' in candidate_arm and 'steps.arm.outputs.explicit_rereview || false' in candidate_arm and 'if jq -e \'any(.labels[]; (.name | ascii_downcase | gsub("[ _-]+"; "-")) == "re-review")\'' in candidate_arm and candidate_arm.index('Re-review label consumption failed') < candidate_arm.index('gh workflow run ai-review-merge.yml'),
...
mutations = {
    ...
    "operator recovery dispatches before consuming label": (gate, arm.replace('if jq -e \'any(.labels[]; (.name | ascii_downcase | gsub("[ _-]+"; "-")) == "re-review")\'', 'if false'), policy),
