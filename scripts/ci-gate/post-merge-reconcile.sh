...
if [ "$HEAD_REPO" = "$TARGET_REPO" ] && [ -n "$HEAD_REF" ] &&
  ...
  if [ "$live_sha" = "$MERGED_HEAD_SHA" ]; then
    delete_err="$(mktemp)"
    if ! gh api --method DELETE "repos/$TARGET_REPO/git/refs/heads/$head_ref_path" \
      >/dev/null 2>"$delete_err"; then
      # Branch cleanup is housekeeping, never a verdict (#458): a repository with
      # auto-delete-branch-on-merge removes the ref itself, and the read above can win
      # that race by a moment, so a 404 here is routine and stays a notice at exit 0.
      # Every other status is not routine, and reporting it as "already absent" was a
      # guess about a result this step never looked at -- it made a cleanup path that had
      # silently stopped working indistinguishable from one that had nothing to do.
      if grep 'HTTP 404' "$delete_err" >/dev/null; then
        echo "::notice::merged head ref already absent or protected"
      else
        echo "::warning::could not delete the merged head ref: $(tr '\n' ' ' <"$delete_err")"
      fi
    fi
    rm -f "$delete_err"
