...
def scenario_bindings(*, should_run, event_name='pull_request', acquisition='success',
...
SCENARIOS = (
    Scenario(
        ...
        bindings=scenario_bindings(should_run='false', acquisition='skipped'),
        functions={'hashFiles': lambda *_: ''},
    ),
    Scenario(
        name='secretless-pr',
        description='pull-request lane, no credentials, allowlisted internal packages',
...
def deferral_is_distinguishable(contract, caller, binds_as=None):
    """Whether a deferred head publishes anything a merge gate can act on.
    ...
    return any(outcome.conclusion not in CONCLUSIONS_A_MERGE_GATE_ACCEPTS
               for outcome in outcomes.values())


def outcomes_for(contract, caller, scenario, binds_as=None):
    bindings = dict(scenario.bindings)
    acquisition = bindings.pop('_acquisition')
    ...
    modelled = model_workflow(contract, resolved)
    # The acquisition job's own result is modelled; the scenario only says what
    # a *failed* acquisition looks like, which the model cannot derive.
    if acquisition != 'success' and 'acquire-secretless-dependencies' in modelled:
        modelled['acquire-secretless-dependencies'].ran = False
    return modelled
        ...
        for contract in (CONTRACT, PROTECTED_CONTRACT):
            for caller in callers_for(contract):
                with self.subTest(contract=contract.name, adopter=caller.name):
                    self.assertTrue(
                        deferral_is_distinguishable(contract, caller),
                        'a deferred head publishes no non-SUCCESS check, so a merge '
                        'gate reading conclusions alone cannot tell a deferred head '
                        'from a verified one — this is Verjson/verjson-ci#184')
    ...
    def test_the_property_fails_against_the_contract_that_shipped_the_defect(self):
        ...
        for caller in callers_for(CONTRACT):
            with self.subTest(adopter=caller.name):
                self.assertFalse(
                    deferral_is_distinguishable(PRE_ADR_0178, caller,
        ...
        for scenario in SCENARIOS[1:]:
            for caller in callers_for(CONTRACT):
                with self.subTest(scenario=scenario.name, adopter=caller.name):
                    ...
                    self.assertTrue(
                        evidence,
                        f'{scenario.name} reports success having executed no work '
                        'step; that is the shape of Verjson/verjson-ci#184')
...
class TheAdopterSurfaceIsTheContractSurface(unittest.TestCase):
    ...
    def test_an_adopter_passing_an_undeclared_input_is_rejected(self):
        drifted = ADOPTERS / '_drifted-adopter.yml'
        drifted.write_text(
