...
grep -qF 'invalid-syntax.yml' "$behavior_script" \
  ...
  && pass "fixtures cover a governed label plus provider-owned invalid workflows" \
  || fail "behavior fixtures omit a governed label or execute caller-controlled scripts"
