...
cp "$root/scripts/gen-container-release.sh" "$root/scripts/container_release_promotion.py" "$root/scripts/container_release_manifest.py" "$root/scripts/container_artifact_extract.py" "$tmp/contract/scripts/"
...
grep -q -- '--bundle-from-oci --predicate-type https://spdx.dev/Document' "$workflow"
