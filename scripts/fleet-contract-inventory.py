...
# Only `pins` and `SHA_RE` are called below. `USES_RE` and `references` are
# deliberate re-exports rather than dead imports: the CI-gate suite asserts the
# two sweeps read one object through this module's name, which is the check that
# makes reintroducing a private copy fail. They are named here so a later reader
# removes them on purpose or not at all.
