...
generated_contract_identity_for_repo() ( # $1 = repo, $2 = audited head or empty
  ...
  read -r mode pin < <(sed -nE \
    's/^# Generated by Verjson\/\.github scripts\/gen-changelog-caller\.sh (generated-artifacts(-with-adr-index)?) ([0-9a-f]{40})$/\1 \3/p' \
    "$tmp/actual/generated-artifacts.yml")
  case "$mode" in generated-artifacts|generated-artifacts-with-adr-index) ;; *)
    echo "::error::phase=audit repo=$repo result=generated-contract-provenance-invalid path=.github/workflows/generated-artifacts.yml"
    return 1 ;;
  esac
  [[ "$pin" =~ ^[0-9a-f]{40}$ ]] || {
...
PY
  ...
  if ! "${generator_env[@]}" "$tmp/gen-changelog-caller.sh" "$mode" "$pin" >"$tmp/expected/generated-artifacts.yml" 2>/dev/null ||
  ...
  for name in generated-artifacts.yml render-next.sh changelog-contract.test.sh release.yml; do
