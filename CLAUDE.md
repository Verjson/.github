...
- [#676](https://github.com/Verjson/.github/issues/676) — Route privileged merge continuations to disposable GitHub-hosted runners. Part 1 (hosted-compute availability and the live lane cutover) remains an owner-confirmed hold. Part 2 is implemented under ADR 0117: the private terminal route consumes the exact caller-supplied lane already validated under ADR 0089, with byte-identical behavior for today's callers. No organization variable or capacity changed.
...
- [#987](https://github.com/Verjson/.github/issues/987) — GitHub Packages retention deletes published `@verjson/*` versions out from under pinned consumers (hardcoded to exactly 3). #1001 (merged) made the count configurable via `VERJSON_PACKAGE_RETENTION_KEEP` (default unchanged — zero live behavior change) and added a rename-deprecation-stub safety net. The actual "stop deleting vs. longer retention window" policy call — a real storage-cost tradeoff — is explicitly left for a human; see the issue comment for the framed decision.
- [#991](https://github.com/Verjson/.github/issues/991) — Terminal merge still requires the long-lived `ORG_ADMIN_TOKEN`; generated callers make it unremovable downstream. A concrete design (mirroring the existing `RELEASE_APP_CLIENT_ID`/`RELEASE_APP_PRIVATE_KEY` pattern) is posted on the issue as of 2026-08-22. Provisioning the new App and granting it bypass-actor status on branch-protection rulesets needs an org admin — not something this session can or should do unprompted. The code-side change (swap `ai-privileged-merge.yml`'s credential) should land in its own tightly-scoped, carefully-reviewed PR once the App exists to test against.
