...
- [#819](https://github.com/Verjson/.github/issues/819) / [#810](https://github.com/Verjson/.github/issues/810) — `AiB` (the only active consumer) landed (#229/#234). Two residual items are explicitly `.github`'s to decide, not AiB's: an ADR 0103 wording correction (in progress) and a `workflow_dispatch`-runs-from-the-selected-ref runtime-bypass finding that needs a mitigation choice (environment reviewers vs. dispatch restriction vs. a ruleset) — no recommendation yet, needs a decision.
