...
- [#341](https://github.com/Verjson/.github/issues/341) — Watchdog kept, not retired (ADR 0056). Re-scope to "no merge-gate job polls on the shared pool": ADR 0053 covers `gate`, `ai-privileged-merge.yml` still routes on `VERJSON_LANE_PRIVILEGED`. It ships **armed** for the proven 35-minute age rule (no `VERJSON_WATCHDOG_DRY_RUN` variable exists, so the workflow supplies `'false'`); the widened #343 poll-step rule is **report-only** until an operator sets `VERJSON_WATCHDOG_POLL_STEP_DRY_RUN=false`. ADR 0058 step 6 shipped 2026-08-06: `gate` skips its poll where the base branch declares a required status check (25 of 91 repos), and polls everywhere else — the watchdog stays load-bearing for `ai-privileged-merge.yml`, which still polls.
