...
- [#1612](https://github.com/Verjson/.github/issues/1612) — `validate_privileged_lane`'s `jq -e` check fails open on an empty/missing `privileged_lane` (a bash here-string never produces truly empty stdin, and jq exits 0 on whitespace-only input regardless of `-e`). Reproduced locally on 2026-09-25 with `bash scripts/ci-gate/privileged-lane-validation.test.sh`, independent of PR #1611.
