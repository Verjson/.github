...
The inspector supports only the canonical generated workflow subset: block mappings
and sequences, flow mappings and sequences, quoted or plain scalars, comments, block
command scalars, and YAML document markers. YAML anchors, aliases (including merge
keys), and explicit tags are intentionally unsupported and make the audit fail closed.
Workflow source is parsed by the repository's standard-library-only
`scripts/required-checks-workflow.py` under isolated Python mode; the audit does
not load an ambient PyYAML installation or provision a package at runtime.
