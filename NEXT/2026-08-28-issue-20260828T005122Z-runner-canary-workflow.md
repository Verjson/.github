...
The canary completes [#629](https://github.com/Verjson/.github/issues/629)'s exact-runner admission path. It verifies the transaction through its unique runner label, database and sibling Docker connectivity, a representative image build, PowerShell, and post-job disk and inode capacity before emitting the digest-bound promotion receipt. Exact disposable-resource absence is proven after cleanup; survivors, inventory failures, and cleanup-phase signals fail closed without leaving promotion evidence. Hosted-selector policy admits its direct label input only at the exact normalized repository-relative dispatch-only workflow path, refuses symlink and non-regular canonical files, and does not widen the global expression allowlist. ADR 0150 defines the `runner-canary-vX.Y.Z` publication and rollback contract.

Runner-routing inventory recognizes the same exact selector through the composed structural policy, including when invoked outside the repository working directory.
