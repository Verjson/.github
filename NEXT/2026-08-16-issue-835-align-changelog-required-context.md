...
The generated contract and PyYAML-backed central audit bind the exact caller mapping at `.github/workflows/changelog.yml`, immutable pin, `with:` inputs, and declared context to that live requirement; duplicate paths, job-level names, matrices, typos, and comment lookalikes fail closed. ADR 0106 supersedes the earlier context-name decision without weakening organization protection, while `workflow` remains a compatibility command for existing automation.
