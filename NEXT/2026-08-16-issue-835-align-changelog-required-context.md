...
The generated contract and standard-library-only central audit bind the exact caller mapping at `.github/workflows/changelog.yml`, immutable pin, exact `with:` inputs, and declared context to that live requirement. Every workflow filename is inspected, including valid YAML document markers, so renamed generated copies, legacy validation callers, duplicate paths, job-level names, matrices, extra inputs, typos, and comment lookalikes fail closed. The staged rollout binds that inspector to merged canonical bytes and rejects override paths before conformance. ADR 0106 supersedes the earlier context-name decision without weakening organization protection, while `workflow` remains a compatibility command for existing automation.
