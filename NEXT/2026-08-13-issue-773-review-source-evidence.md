...
Maintainer-authorized `re-review` labels now bind one explicit diagnostic pass into the arm receipt, publish a distinct App reservation marker, count toward telemetry, and suppress automatic fallback without weakening the normal two-pass ceiling. When GitHub creates no run for a label event, rerunning an existing exact-head arm safely revalidates current PR state and issues a fresh receipt; this recovery does not claim to repair GitHub event delivery.
