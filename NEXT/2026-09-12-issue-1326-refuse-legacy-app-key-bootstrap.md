...
The manifest example, adopter runbook, ADR 0125 amendment, and boundary-mocked tests document the exact classification and route owners to the main-only environment provisioning path. Provisioning receipts carry the rejected key identifier and only the non-secret repository-relative path to that runbook; they never include secret values.
