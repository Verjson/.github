# 0071 — Changelog impact governs version bumps
...
The release command can select an explicit subset of fragments. Impact therefore
has to be evaluated after selection: an unselected breaking fragment must not
force a major release of an unrelated selected patch.
...
Prefixes such as `v` and `python-v` identify independent histories. Ordinary
SemVer axes apply to `0.x`; breaking impact has no special pre-1.0 exception.
The first release in a prefix history establishes its baseline because there is
...
rendered release notes. Workflows pass the requested version and fragment
selectors to the engine; consumers must not implement a second parser or bump
calculator.
...
- Mixed selections use their highest impact, while subset releases ignore
...
- **Validate in generated workflows.** Duplicate parsers drift and bypass the
  canonical engine, especially after explicit fragment selection.
