...
Generated consumers install two thin immutable delegates from
`gen-privileged-merge-caller.sh`: the dispatch-only privileged caller and a
`workflow_run: completed` retry caller naming that repository's deterministic CI
workflows. The retry delegates to the canonical resolver and cannot invoke paid
review. Their contract gains no polling option or new dependency. The retained fleet
watchdog remains an independent safety brake for legacy or misconfigured workloads,
but its production merge-gate allowlists stay inert while this structural contract
holds.
