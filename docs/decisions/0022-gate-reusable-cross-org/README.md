...
## Amendment (2026-08-05) — `runner_labels` is optional again; its premise expired

The 2026-07-23 item above is reversed for `runner_labels` only. The
`require_secrets` step is untouched. This restores ADR 0041's invariant rather
than revisiting the reusable-distribution decision.

That item rested on a fact that is no longer true: a caller omitting
`runner_labels` "fell through the `runs-on` fallback" to `self-hosted,gate`, a
pool no consumer org has a runner for, so the job queued forever with no error.
Since ADR 0040/0041 (#401) every `runs-on` here ends at
`vars.VERJSON_LANE_FALLBACK || '["ubuntu-24.04"]'`. An org with no lane variables
now lands on a hosted runner that exists, so the failure the requirement
prevented cannot occur, and the requirement's own cost became the live defect:
...
- That is exactly the coupling ADR 0041 removes: a relabel becomes a pull request
  in ~90 consumer repositories instead of an org-variable flip. #401 cleared every
  `runs-on:` in this repository; the generator was the last producer of fleet
  labels the org owns.
...
The input itself is **kept**, and the `runs-on` precedence is unchanged
(`inputs.runner_labels` still wins). A self-hosted consumer outside Verjson has
no lane variables to fall through to, and it is the only caller that still needs
to name its own fleet; the generator therefore takes the labels as an optional
argument and emits the input only when asked.

Pinned by `reusable-workflow.test.sh` (now asserting `required: false` with this
rationale), `privileged-merge-caller-contract.test.sh` (a generated caller
contains no `self-hosted` literal and no `runner_labels` entry, while an explicit
fleet is still forwarded) and `runner-routing-policy.test.sh`, which evaluates the
real `runs-on` expression for both polarities: omitted → `VERJSON_LANE_PRIVILEGED`,
supplied → the caller's labels.

Consumer blast radius: existing generated callers keep working — they pass a
still-accepted input — but they stay label-pinned until regenerated by the #365
sweep, which must therefore land after this. See
[#405](https://github.com/Verjson/.github/issues/405).
