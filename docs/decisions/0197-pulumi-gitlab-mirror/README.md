...
4. Keep the already-published v1 and v2 manifest schemas and artifacts immutable. Publish the Pulumi artifact through a new manifest schema version with explicit Pulumi paths and digests. Adapt consumers to that version; retain v1/v2 parsing where needed to verify already-published releases. Never describe historical Terraform artifacts as Pulumi or rewrite a released manifest.
