...
- **Date:** 2026-07-30
- **Issues:** [Verjson/.github#249](https://github.com/Verjson/.github/issues/249), [Verjson/.github#524](https://github.com/Verjson/.github/issues/524)
...
## Amendment (2026-08-07) — dependency updates carry release context (#524)

A dependency manifest or lockfile change is an observable repository change even
when generated by Renovate. `check-pr` now requires that such a diff add a new
valid `NEXT/` fragment. The rule uses the same Git diff boundary as the existing
released-history and fragment-consumption checks; it has no actor or bot
exception, and modifying an unrelated pre-existing fragment does not satisfy it.

The filename boundary is deliberately enumerated by package ecosystem rather
than inferred from broad extensions: Node package manifests and npm, pnpm, and
Yarn locks; Python project, requirements, Poetry, Pipenv, and uv files; Go
modules; Cargo manifests and locks; and Terraform's exact
`.terraform.lock.hcl`. Similar documentation and backup names remain outside the
rule. An unreadable or malformed base/head revision still fails closed through
the contract's Git boundary.

Adopters receive this only by regenerating the workflow, renderer, and contract
test from `scripts/gen-changelog-caller.sh` at one immutable contract SHA. A
handwritten bot exception or local approximation would weaken the organization
