...
| Required reusable actionlint | per PR, every Verjson caller | no literal `ubuntu-*`, `macos-*`, or `windows-*` selector | none |
...
The reusable `actionlint.yml` check implements the hosted-selector policy from its own immutable
...
Its consumer mode rejects literal Linux selectors as well as metered macOS and Windows families,
while retaining this repository's visibility, OS-lane timeout, and sanctioned-release rules as
local policy. Literal portability tails inside reviewed lane expressions remain valid. Unreviewed
selector expressions still fail closed so a caller cannot assemble a hidden hosted family with
`format`, `join`, or an arbitrary input, variable, or needs output.
